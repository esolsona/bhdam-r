...
# --------------------------------------------------------------------------- #
# Segmented AEAD (v1.4): AES-256-GCM has a hard per-invocation limit of
# 2**31-1 bytes. To support arbitrarily large payloads with bounded per-call
# memory, the package is sealed in independent 1 GiB segments. Each segment
# uses a distinct nonce derived from a fresh per-transfer base nonce and the
# segment index, and binds (index, total) into its AAD so that reordering or
# truncation of segments is detected. Payloads below the segment size produce a
# single segment, so the format is uniform for all sizes.
# --------------------------------------------------------------------------- #
AEAD_SEGMENT = 1 << 30  # 1 GiB

def _segment_nonce(base_nonce12: bytes, idx: int) -> bytes:
    """12-byte nonce = 4 fixed bytes from the per-transfer base + 8-byte counter."""
    return base_nonce12[:4] + struct.pack(">Q", idx)

def aead_seal_segmented(key: bytes, base_nonce12: bytes, data: bytes, aad: bytes) -> bytes:
    aead = AESGCM(key)
    n_seg = max(1, (len(data) + AEAD_SEGMENT - 1) // AEAD_SEGMENT)
    out = struct.pack(">I", n_seg)
    for i in range(n_seg):
        seg = data[i * AEAD_SEGMENT:(i + 1) * AEAD_SEGMENT]
        seg_aad = aad + struct.pack(">II", i, n_seg)
        ct = aead.encrypt(_segment_nonce(base_nonce12, i), seg, seg_aad)
        out += struct.pack(">Q", len(ct)) + ct
    return out

def aead_open_segmented(key: bytes, base_nonce12: bytes, blob: bytes, aad: bytes) -> bytes:
    aead = AESGCM(key)
    (n_seg,) = struct.unpack(">I", blob[:4]); off = 4
    parts = []
    for i in range(n_seg):
        (clen,) = struct.unpack(">Q", blob[off:off + 8]); off += 8
        ct = blob[off:off + clen]; off += clen
        seg_aad = aad + struct.pack(">II", i, n_seg)
        parts.append(aead.decrypt(_segment_nonce(base_nonce12, i), ct, seg_aad))
    return b"".join(parts)
...
class Sender:
    ...
    def build_transfer(
        ...
        ciphertext = aead_seal_segmented(dek, aead_nonce, package, tid.encode())
        _t.stop("encrypt_ms")
        _t.start()
...
class Recipient:
    ...
    def receive(
        ...
        _t.start()
        try:
            plaintext = aead_open_segmented(
                dek, bytes.fromhex(manifest.aead_nonce), ciphertext,
